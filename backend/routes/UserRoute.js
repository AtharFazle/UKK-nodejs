    ...
    await user.findOne({ where: params })
        .then(result => {
            if (result) {
                ...
                res.json({ success: 1, message: "Login success, as admin", token: token })
            } else {
                res.json({ success: 0, message: "login failed check your email or password!" })
            }
        })
        .catch(error => res.json({ message: error.message }))
});
        ...
        .then(result => {
            try {
                if (result) {
                    let payload = JSON.stringify(result);
                    let token = jwt.sign(payload, SECRET_KEY)
                    res.json({ success: 1, message: "login succcess as resepsionis", token: token })
                } else {
                    res.json({ success: 0, message: "login failed check your email or password!" })
                }
            } catch (error) {
